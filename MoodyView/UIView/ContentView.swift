...
struct ContentView: View {
    ...
    @State private var TitleString: String = "Add Mode"
    ...
    @State private var beforeaSelectedScreenMod: String = ""
    ...
    var body: some View {
        NavigationSplitView {
            ...
            if isAddingNewMode {
                ...
                .onAppear {
                    print("new mode editor appeared")
                    beforeaSelectedScreenMod = "NewMod"
                }
            }
                ...
                .onAppear {
                    beforeaSelectedScreenMod = "Mod detail"
                }
            }
            else {
                Text("Selecr Mode Or Add New Mode")
        ...
        .onChange(of: tempSelectedMode) { oldValue, newValue in
            appState.selectedMode = tempSelectedMode
            if isAddingNewMode {
                cancelNewModeAdding()
            }
            TitleString = newValue.flatMap { modeDetails[$0]?.name } ?? "모드 추가"
        }
    }
